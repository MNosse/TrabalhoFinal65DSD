...
public class JUserIcon extends JPanel {
    ...
    public JUserIcon(Color color, int minWidth, int maxWidth, int height, boolean isOnline, int marginBottom, String username) {
        this.color = color;
        this.height = height;
    ...
    private void initialize() {
        ...
        setVisible(true);
        setBackground(color);
        setMinimumSize(new Dimension(minWidth, height+marginBottom));
        setMaximumSize(new Dimension(maxWidth, height+marginBottom));
        setPreferredSize(getMinimumSize());
        setSize(getPreferredSize());
        setBorder(new EmptyBorder(0, 0, marginBottom, 0));
        //JLABEL lblIcon
        lblIcon = new JLabel(username.substring(0, 1));
    ...
    public void notifyOnline() {
        lblIcon.setIcon(new ImageIcon(ImagesInstances.USER_ON_LBL.getScaledInstance(height, height, Image.SCALE_SMOOTH)));
    }
    
    public void notifyOffline() {
        lblIcon.setIcon(new ImageIcon(ImagesInstances.USER_OFF_LBL.getScaledInstance(height, height, Image.SCALE_SMOOTH)));
    }
